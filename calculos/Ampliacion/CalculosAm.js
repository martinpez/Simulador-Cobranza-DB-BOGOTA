function CalculosAmpliacion() {
  ...
  // 1. Cargar valores del cliente inmediatamente en el formulario (síncrono)
  ...
  if (esCampana) {
    ...
    procesarCalculosAmpliacion();
  } else {
    sessionStorage.campanaAmpliacion = 'No';
    // Consulta asíncrona de tasas
    consultarTasasAmpliacion(e.dataItem.EdadMoraCl);
  }
}

// Función asíncrona encargada únicamente de realizar la consulta a la base de datos
async function consultarTasasAmpliacion(edadMora) {
  let dbPorcMora = 0;
  let dbPorcCte = 0;
  try {
    ...
    if (response && response[0] && response[0][0]) {
      ...
      console.warn('No se encontraron tasas de ampliación para la edad de mora:', edadMora);
    }
  } catch (error) {
    console.error('Error al consultar tasas de ampliación:', error);
  }

  // Enviar las tasas obtenidas a la función normal
  aplicarTasasNormales(dbPorcCte, dbPorcMora);
}

// Función normal (síncrona) que recibe las variables ya cargadas
function aplicarTasasNormales(dbPorcCte, dbPorcMora) {
  debugger;
