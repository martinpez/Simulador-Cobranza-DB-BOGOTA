...
function DataFunConsolidacion(mecanismo) {
    ...
    // CAPTURA DE OBLIGACIONES DESDE TOGGLES ACTIVOS
    function getObligacionesActivas() {
        ...
        cards.forEach((card) => {
            ...
            const nombreDesdeAttr = card.querySelector("[data-obligacion-nombre]")
                ?.getAttribute("data-obligacion-nombre");
            const nombreDesdeInput = card.querySelector("input[data-label*='Obligación']")?.value
                || card.querySelector("input.nombre-obligacion")?.value
                || card.querySelector("input[placeholder*='obligación']")?.value;

            const numObligacion = nombreDesdeAttr
                || nombreDesdeInput
    ...
    // INSERCIÓN DINÁMICA EN LA TABLA DEL FUN Genera las filas según las obligaciones encontradas sin límite fijo
    const formateador = new Intl.NumberFormat("es-CO", {
        style: "currency",
        currency: "COP",
        minimumFractionDigits: 0,
    ...
    function llenarFilasObligaciones(obligaciones) {
        const contenedor = document.getElementById(
            `filas-obligaciones-${mecanismo}`
        );

        if (!contenedor) {
            console.warn(
                `Contenedor de filas no encontrado: #filas-obligaciones-${mecanismo}`
            );
            return;
        }

        // Limpia filas previas antes de redibujar
        ...
        obligaciones.forEach((obl, index) => {
            ...
            fila.innerHTML = `
                <div class="grid-cell_con" id="numObligacion_${i}_${mecanismo}">
                    ${obl.numObligacion}
                </div>
                <div class="grid-cell_con" id="saldoTotal_${i}_${mecanismo}">
                    ${formateador.format(obl.saldoTotal)}
                </div>
                <div class="grid-cell_con" id="intCorrientes_${i}_${mecanismo}">
                    ${formateador.format(obl.intCorrientes)}
                </div>
                <div class="grid-cell_con" id="intMora_${i}_${mecanismo}">
                    ${formateador.format(obl.intMora)}
                </div>
                <div class="grid-cell_con" id="intExtraC_${i}_${mecanismo}">
                    ${formateador.format(obl.intExtraC)}
                </div>`;
        ...
        console.log(
            `DataFunConsolidacion — ${obligaciones.length} fila(s) generadas en tabla.`
        );
    }

    //CARGA DE DATOS GENERALES EN EL FUn
    function loadFormData(data) {
